...
| `src/playback.zig` | Interactive play: raw-mode TUI over `player.zig` — one composed frame per tick (hidden cursor, sync bracket) with the colored layered waveform at the start of one scrollable page, the playhead line, the time ruler, the complete wrapped Markdown transcript below it, SPACE pause, ←/→ and SHIFT+←/→ seek, ↑/↓/PgUp/PgDn/Home/End transcript scroll, I/O region anchors (the span recolored between two anchor lines), DELETE with ENTER confirmation cutting via `cut.zig`, R reset, T jumps to/transcribes the text, Y copies, S shares, Q stops; a confirmed cut reloads the shortened audio and keeps the player open; blocking fallback off a tty |
...
| `src/transcribecmd.zig` | The transcribe verb: resolve the selection, send the recording to Deepgram, render timestamped OKF markdown, run the LLM refine pass (shared by the CLI and the play view's `T`), then open the Markdown viewer |
